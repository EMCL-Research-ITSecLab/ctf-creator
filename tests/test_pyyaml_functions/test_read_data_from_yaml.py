...
def test_valid_data():
    ...
    expected = (['nginx', 'redis'], ['user1', 'user2'], ['/path/to/key'], ['admin@10.0.0.0', 'ubuntu@10.20.30.101'], 192, 168, 1)
...
def test_missing_field():
    ...
    with pytest.raises(ValueError, match="Missing subnet_third_part field in YAML data"):
...
def test_empty_subnet_field():
    ...
    with pytest.raises(ValueError, match="Expected 'subnet_first_part' to be a non-empty list"):
        read_data_from_yaml(data)
...
def test_subnet_part_contains_multiple_values():
    ...
    with pytest.raises(ValueError, match="Expected 'subnet_first_part' to contain exactly one value"):
        read_data_from_yaml(data)
...
def test_subnet_part_non_integer_value():
    ...
    with pytest.raises(ValueError, match="Expected 'subnet_first_part' to contain an integer value"):
        read_data_from_yaml(data)
...
def test_extra_fields():
    ...
    result = read_data_from_yaml(data)
    expected = (['nginx'], ['user1'], ['/path/to/key'], ['ubuntu@10.20.30.101'], 192, 168, 1)
    assert result == expected
...
def test_invalid_host_format():
    ...
    with pytest.raises(ValueError, match="Expected 'hosts' entries to be in the format 'username@ip_address', but got 'invalid-host'"):
        read_data_from_yaml(data)
