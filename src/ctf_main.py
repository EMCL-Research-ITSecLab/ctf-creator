...
def main(config, save_path):
    ...
    try:
        ...
        # Process the YAML data as needed for the CTF-Creator
            containers, users, key, hosts, subnet_first_part, subnet_second_part, subnet_third_part = yaml_func.read_data_from_yaml(
                data)
        ...
        for host in hosts:
            try:
                ...
                raise RuntimeError(f"An error occurred while fetching the Docker API version on host {host}. "
                ...
                raise RuntimeError(f"An unexpected error occurred while connecting to Docker on host {host}. "
                                   f"This may indicate that Docker is not installed, not running, or not properly configured on the host. "
                                   f"Please verify the following: "
                                   f"1. Docker is installed and running on the host by using the command: 'docker info'. "
                                   f"2. The Docker daemon is configured to accept remote connections if you are accessing it remotely. "
        ...
        for k in range(len(users)):
            ...
            if not os.path.exists(local_save_path_to_user):
                doc.create_network(
                    docker_client, network_name, subnet, gateway)
                click.echo(
                    f"For the user: {user_name}, an OpenVPN configuration file will be generated!")
                doc.create_openvpn_server(docker_client, network_name, user_name,
                                          f"{subnet_first_part}.{subnet_second_part}.{subnet_base}.2", k, current_host)
                ...
                ovpn_func.modify_ovpn_file(
                    f"{save_path}/data/{user_name}/client.ovpn", 1194+k, new_push_route)
                # Writes a readmefile which describes reachable docker containers
                readme.write_readme_for_ovpn_connection(
                    local_save_path_to_user, f"{subnet_first_part}.{subnet_second_part}.{subnet_base}", containers)
            # Starts OpenVPN-Container with existing data in save_path
                ...
                existing_host_ip, existing_port_number, existing_subnet = hosts_func.extract_ovpn_info(
                ...
                if existing_host_ip in extracted_hosts:
                    ...
                    changed_ovpn_username = ovpn_func.modify_ovpn_file_change_host(
                        f"{save_path}/data/{user_name}/client.ovpn", current_host, existing_port_number, user_name)
                    if changed_ovpn_username:
                        new_changed_ovpn_files_users.append(
                            changed_ovpn_username)
                # Send existing openvpn_data to hosts
                hosts_func.send_and_extract_tar_via_ssh(f"{save_path}/data/{user_name}/dockovpn_data.tar", existing_host_username,
                                                        existing_host_ip, f"/home/{existing_host_username}/ctf-data/{user_name}/dock_vpn_data.tar")
                time.sleep(2)
                ...
                doc.create_network(
                    docker_client, network_name, f"{existing_subnet}.0/24", f"{existing_subnet}.1")
                time.sleep(1)
                # Creates Openvpn server with existing data
                doc.create_openvpn_server_with_existing_data(
                    docker_client, network_name, user_name, f"{existing_subnet}.2", existing_port_number, existing_host_ip, f"/home/{existing_host_username}/ctf-data/{user_name}/Dockovpn_data/")
                click.echo(
            ...
            for i, element in enumerate(containers):
                ...
                doc.create_container(docker_client, network_name, user_name +
                                     f"_{container_name}_" + f"{i}", f"{element}", f"{subnet_first_part}.{subnet_second_part}.{subnet_base}.{3+i}")
