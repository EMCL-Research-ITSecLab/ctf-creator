...
def main(config, save_path):
    ...
    try:
        ...
        click.echo(
            "Start to clean up the Hosts. Deletes old Docker-containers and networks"
        )
        for host in hosts:
        ...
        for k in range(len(users)):
            ...
            # Starts OpenVPN-Container with existing data in save_path
                ...
                hosts_func.send_and_extract_tar_via_ssh(
                    ...
                    extracted_hosts_username[current_vm - 1],
                    extracted_hosts[current_vm - 1],
                    f"/home/{extracted_hosts_username[current_vm-1]}/ctf-data/{user_name}/dock_vpn_data.tar",
                )
                time.sleep(2)
                ...
                doc.create_openvpn_server_with_existing_data(
                    ...
                    k,
                    current_host,
                    f"/home/{extracted_hosts_username[current_vm-1]}/ctf-data/{user_name}/Dockovpn_data/",
                )
                ovpn_func.modify_ovpn_file_change_host(
                    f"{save_path}/data/{user_name}/client.ovpn", current_host, 1194 + k
                )
                click.echo(
